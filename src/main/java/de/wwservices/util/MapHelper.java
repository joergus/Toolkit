...
public class MapHelper
{
      ...
      Map<K, V> incomingMap)
  {
    ...
    Set<Entry<K, V>> sortedSet = new TreeSet<Entry<K, V>>(entrySetComparator);
      ...
      Map<K, V> incomingMap, Comparator<V> comparator)
  {
    ...
    Comparator<Entry<K, V>> entrySetComparator = new EntrySetComparator2<K, V>(comparator);
    Set<Entry<K, V>> sortedSet = new TreeSet<Entry<K, V>>(entrySetComparator);
    for (Entry<K, V> entry : incomingMap.entrySet())
    {
      sortedSet.add(entry);
   ...
   * @param <V>
   */
  private static class EntrySetComparator2<K extends Comparable<K>, V> implements Serializable,
